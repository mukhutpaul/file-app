...
const getStatusBadge = (status : string) =>{
    switch(status){
        ...
        default:
           return <div className='badge badge-danger'>Statut inconnu</div>
    }
}
...
const TicketComponent : FC<TickeComponentProps> = ({ticket,index,totalWaitingTime=0}) => {
    ...
    const totalMinnutes = totalWaitingTime % 60
    const formatedTotalWaitTime = `${totalHours}h ${totalMinnutes}min`
    ...
    useEffect(()=>{
        ...
        const waitTimeInMinute = (currentTime - createdTime) / 60000

        const hours = Math.floor(waitTimeInMinute / 60) 
        const minutes = Math.floor(waitTimeInMinute % 60)
        setFormattedRealWaitTime(`${hours}h ${minutes}min`)

        if(totalWaitingTime !== 0){
            if(waitTimeInMinute > totalWaitingTime){
        ...
        <div className='border p-5 border-base-300 rounded-xl flex flex-col space-y-2'>
            ...
            {ticket.status !== "IN_PROGRESS" && ticket.status !== "FINISHED" &&(
                <div className='border border-base-300 rounded-xl p-5'>
                    <span className='badge badge-accent badge-outline'>Atente</span>
                    <ul className="timeline timeline-vertical lg:timeline-horizontal w-full">
                        ...
                        {totalWaitingTime !== 0 &&(
                            ...
                            <div className={`timeline-end timeline-box border-2 ${waitTimeStatus==="success"?"border-green-500":"border-red-500"}`}>{formatedTotalWaitTime}</div>
